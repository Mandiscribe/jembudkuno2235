	    	...
	    	if (extended) {
	    		...
	    		if (func == "OnRequestWorldSelectMenu") {
	    			...
	    			if (ctx.find("add_label_with_icon|big|`wGet a GrowID``|left|206|") != std::string::npos) {
	    				SendPacket(2, "action|dialog_return\ndialog_name|growid_apply\nlogon|"+ Growid_acc + to_string(MULAI_DARI) + "\npassword|"+ Password_acc +"\npassword_verify|"+ Password_acc +"\nemail|"+ Gmail_acc + to_string(START_DARI) + "@gmail.com", peer);
	    				MULAI_DARI = MULAI_DARI + 1;
	    				...
	    				cout << "Success Make Account Growid: " << growid << endl;
	    				appendAcc("User: " + growid + ",Password: " + passwr);
